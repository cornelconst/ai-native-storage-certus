...
**Created**: 2026-04-16  
**Status**: Revised (drift sync 2026-04-16)  
...
- What happens during a crash while a new slab is being allocated? The superblock is written last; if the crash occurs before the superblock update, the partially written slab is invisible to recovery.
...
- **Size Class**: A valid extent size (128 KiB to 5 MiB, multiples of 4 KiB). Size classes are not pre-declared at initialization; any valid size can be used in `create_extent` and will trigger slab allocation on demand.
...
- **Superblock**: The management header stored at block 0 on the device. Contains total device blocks, slab size, a slab table (mapping each slab to its size class and start LBA), and a CRC-32 integrity check.
